...
const App = () => {
  ...
  return (
    <div className={`min-h-screen ${darkMode ? 'bg-gray-900' : 'bg-gray-100'}`}>
        ...
        <div className="container mx-auto p-4 flex justify-between items-center">
          ...
          <ul className="flex items-center space-x-6">
            {['Inicio', 'Portafolio', 'Contacto'].map((item) => (
              <li key={item}>
                <button
                  className={`relative px-4 py-2 text-sm font-semibold transition-all duration-300 rounded-md ${
          ...
          <button
            className={`rounded-full p-3 transform transition-transform duration-300 ${
              darkMode
      ...
      <div
        className={`absolute top-0 left-0 w-full h-16 z-0 blur-lg opacity-40 ${
          darkMode
      ...
      <footer className={`py-4 ${darkMode ? 'bg-gray-800' : 'bg-gray-200'}`}>
        <div className="container mx-auto text-center">
          <p className={`text-sm ${darkMode ? 'text-gray-200' : 'text-gray-800'}`}>
            2023 Sandyslein Correa. Todos los derechos reservados.
          </p>
        </div>
      </footer>
    </div>
